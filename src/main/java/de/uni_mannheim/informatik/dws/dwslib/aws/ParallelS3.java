...
import de.uni_mannheim.informatik.dws.dwslib.aws.S3Permission.PermissionType;
import de.uni_mannheim.informatik.dws.dwslib.aws.S3Permission.UserGroup;
...
public class ParallelS3 {
	...
	public static void main(String[] args)
	{
		...
		try {
			...
			if(cmd.getArgs()[0].equals("setacl") && cmd.getArgs().length==3)
			{
				...
				S3Permission p = new S3Permission(UserGroup.valueOf(grp), PermissionType.valueOf(perm));

				processor = new ParallelS3SetAcl(numThreads, cred, target, p);
			}
			if(cmd.getArgs()[0].equals("ls"))
			{
				S3Helper s3 = new S3Helper(cred.getAwsAccessKey(), cred.getAwsSecret());
				
				List<S3File> lst = target.loadList(s3);
				
				for(S3File f : lst)
				{
					System.out.println("s3://" + f.get_bucket() + "/" + f.get_key());
				}
	...
	private static void usage(Options options)
	{
		...
		formatter.printHelp("ParallelS3 [options] <command> <arguments...>", options);
	...
	private static CommandTarget parseTarget(CommandLine cmd) throws Exception
	{
		...
		if(cmd.hasOption("targetfilter"))
		{
			...
			String url_string = cmd.getOptionValue("targetfilter").replace("s3://", "http://");
			URL u = new URL(url_string);
			S3FileFilter f = new S3FileFilter(u.getHost(), u.getPath().replaceFirst("/", ""));
